const supertest = require('supertest');
const app = require('../app');

const server = app.listen();
const request = supertest.agent(server);

describe('GET /v1/merchant-category', () => {
  const token = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..iuPQn1d860XJgH3vgBUue9smUc-Xeg1pdZgvBhTn0ek';

  it('should not be able to consume the route /v1/merchant-category since no token was sent', (done) => {
    request
      .get('/v1/merchant-category')
      .expect(401, done);
  });

  it('should return merchant category data', (done) => {
    request
      .get('/v1/merchant-category')
      .set('Accept', 'application/json')
      .set('Authorization', `Bearer ${token}`)
      .expect(200)
      .end((err) => {
        if (err) return done(err);

        return done();
      });
  });
});
